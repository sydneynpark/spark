#!/usr/bin/env python3
"""
Iterate over every book review in an S3 bucket and invoke a (private) Lambda
...
# Cover images the lambda itself writes back into this bucket. They aren't
# review files and would fail to parse as UTF-8 markdown if (re)processed.
...
def main():
    ...
    for page in paginator.paginate(Bucket=BUCKET_NAME):
        for obj in page.get("Contents", []):
            ...
            # Skip cover images the lambda writes back into this bucket.
            if key.startswith(COVERS_PREFIX):
                continue

            # Review files are the only thing the lambda processes.
